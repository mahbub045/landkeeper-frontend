...
import { useAcceptInvitationMutation } from '@/store/api/endpoints/client/Common/Tenant/TenantsApi';
...
interface AcceptInvitationApiError {
  data?: {
    detail?: string;
    message?: string;
...
function TenantAcceptInviteContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await acceptInvitation({ payload, token }).unwrap();
      toast.success('Invitation accepted successfully');
      router.push('/auth/signin');
    } catch (error: unknown) {
      let message = 'Failed to accept invitation. Please try again.';
      if (error && typeof error === 'object' && 'data' in error) {
        const apiError = error as AcceptInvitationApiError;
        const { data } = apiError;
        if (data) {
      ...
      <div className='flex min-h-screen'>
        ...
        <div className='flex w-full items-center justify-center bg-white p-8 lg:w-1/2 dark:bg-gray-900'>
          <div className='w-full max-w-md text-center'>
            ...
            <Link
              href='/auth/signin'
              className='text-primary/80 hover:text-primary mt-6 inline-block text-sm font-medium'
            >
              Back to sign in
            </Link>
          </div>
      ...
      <div className='flex w-full items-center justify-center bg-white p-8 lg:w-1/2 dark:bg-gray-900'>
        <div className='w-full max-w-md'>
          ...
          <form onSubmit={handleSubmit} className='space-y-5'>
            ...
            >
              {acceptInvitationLoading && <Loading className='text-white!' />}
              Accept invitation
            </Button>
          </form>
