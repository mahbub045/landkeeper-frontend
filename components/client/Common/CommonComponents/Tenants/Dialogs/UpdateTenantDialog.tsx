...
function tenantToForm(tenant: TenantTypes): TenantForm {
  return {
    ...
    deposit: tenant.deposit ? String(tenant.deposit) : '',
    tenancy_start_date: tenant.tenancy_start_date
      ? String(tenant.tenancy_start_date)
      : '',
    tenancy_end_date: tenant.tenancy_end_date
      ? String(tenant.tenancy_end_date)
      : '',
    employment_details: tenant.employment_details ?? '',
    guarantor_name: tenant.guarantor_name ?? '',
    notes: tenant.notes ?? '',
  };
}

// ── Outer shell: owns the Dialog, keys the form by tenant so it remounts
// with fresh state instead of syncing via an effect. ─────────────────────────

const UpdateTenantDialog: React.FC<UpdateTenantModalProps> = ({
  open,
  onClose,
  onSuccess,
  tenant,
...
// ── Inner form: remounts whenever `tenant.alias` changes, so every piece
// of state below is naturally initialized from the current tenant. ──────────

const UpdateTenantForm: React.FC<UpdateTenantFormProps> = ({
  tenant,
  onClose,
  onSuccess,
}) => {
      ...
      >
                    ...
                    {data.map((p: Property) => (
                      <li
                        ...
                        onMouseDown={() => {
                          ...
                          setSelectedPropertyName(p.property_name);
                          setPropertySearch('');
        ...
        <div className='grid grid-cols-2 gap-4'>
          ...
          <Field data-invalid={!!fieldErrors.tenancy_end_date}>
            ...
            />
            <FieldError errors={[{ message: fieldErrors.tenancy_end_date }]} />
          </Field>
        </div>
        ...
        <Field data-invalid={!!fieldErrors.employment_details}>
          ...
          />
          <FieldError errors={[{ message: fieldErrors.employment_details }]} />
        </Field>
