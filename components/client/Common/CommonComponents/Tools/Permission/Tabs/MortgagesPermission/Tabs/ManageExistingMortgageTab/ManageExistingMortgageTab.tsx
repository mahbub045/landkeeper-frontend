...
const ManageExistingMortgageTab: React.FC<ManageExistingMortgageTabProps> = ({
  isLoadingGranted,
  userAlias,
  grantedMortgages,
  grantedCount,
  ...
  return (
    <>
      ...
      {!isLoadingGranted && userAlias && grantedMortgages.length > 0 && (
        <>
          <div className='grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3'>
            {grantedMortgages.map((item: MortgagesPermissionType) => {
      ...
      {!isLoadingGranted && userAlias && grantedMortgages.length === 0 && (
        <Card className='text-muted-foreground border-dashed p-8 text-center text-sm shadow-none'>
          This user has no mortgage permissions yet.
        </Card>
      )}
    </>
  );
...
export default ManageExistingMortgageTab;
